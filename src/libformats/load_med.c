			...
			long offs, int blks, const char *id)
{
  U_LNG		*blokData= (U_LNG *)ALLOCATE_CLRMEM(blks, U_LNG,
				"load_med::load_mmd_blocks(blokData)"),
		*xblkData= (U_LNG *)ALLOCATE_CLRMEM(blks, U_LNG,
				"load_med::load_mmd_blocks(xblkdata)");
  int		*blokNlen= (int *)ALLOCATE_CLRMEM(blks, int,
				"load_med::load_mmd_blocks(bloknlen)"),
		i;
  EkErrFlag	error= ( (blokData == NULL) || (xblkData == NULL)
	...
	FREE_MEMORY(blokData, "load_med::load_mmd_blocks(blokData)");
	FREE_MEMORY(xblkData, "load_med::load_mmd_blocks(xblkData)");
	FREE_MEMORY(blokNlen, "load_med::load_mmd_blocks(bloknlen)");
  return error;
}
...
{
  U_LNG		*sampData= (U_LNG *)ALLOCATE_CLRMEM(lastinst,
			sizeof(U_LNG), "load_med::load_mmd_instrs()");
  EkErrFlag	error= sampData? fputat(ifPtr, offs)
	...
	FREE_MEMORY(sampData, "load_med::load_mmd_instrs()");
  return error;
}
