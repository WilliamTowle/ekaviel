...
EkErrFlag ekinstr_destroy(EkInstr *inst)
{
	...
	{
#ifdef DEVEL
;fprintf(stderr, "- INST::name %s %08x\n", EKINSTR_FIELD(*inst, name),
					(int)EKINSTR_FIELD(*inst, name));
#endif
	    if (EKINSTR_FIELD(*inst, name) != NULL)
		FREE_MEMORY(EKINSTR_FIELD(*inst, name),
			"ekinstr::ekinstr_destroy(name)");
	    if (EKINSTR_FIELD(*inst, waveform) != NULL)
		FREE_MEMORY(EKINSTR_FIELD(*inst, waveform),
			"ekinstr::ekinstr_destroy(waveform)");
	    return EK_ERR_NONE;
	}
}
